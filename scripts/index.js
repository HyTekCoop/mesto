...
// это конечно не ошибка, но я старался все попапы сделать через одни функции закрытия и открытия,
// то есть у меня было только одна функция закртыия и открытия для всех попапов
// но у меня это не вышло, идея была в передаче функции аргумента попапа кпримеру  popupChange или  popupAdd,
// реализовать у меня это не получилось, если это возможно подскажите пожалуйста как это сделать или намек на это ;)
// но у меня еще возник вопрос в том что в инпуты попапов подается разная информация, вот функция закрытия точно должна быть одной для всех попапов
